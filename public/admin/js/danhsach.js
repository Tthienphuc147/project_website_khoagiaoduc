	...
	str += '<div class="modal-dialog">'
		str += '<div class="modal-content">'
			str += '<div class="modal-header header-color-modal bg-color-3" style="background: #2b982b; color: white;">'
				str += '<h4 class="modal-title">Thêm danh mục bài viết</h4>'
				...
				str += '<form id="form-add" method="POST" enctype="application/x-www-form-urlencoded">'
					str += '<div class="row">'
						str += '<div class="form-group col-md-12">'
							str += '<label>Tên danh mục bài viết</label>'
							str += '<input type="text" class="form-control" placeholder="Nhập tên danh mục bài viết" name="ten" onfocus="focusten(this)">'
...
function saveAdd(){
	...
	$.ajax({
        type: 'POST',
        url: 'quantri/danhmucbaiviet/them',
        headers: {
        ...
        success: (response) => {
            if(response.status){
            	let strOption = ''
            	strOption += '<button title="Chỉnh sửa" class="pd-setting-ed" onclick="editID('+ response.danh_muc_bai_viet.id + ', this)">'
            		strOption += '<i class="fa fa-pencil-square-o" aria-hidden="true"></i>'
            	strOption += '</button>'
            	strOption += '<button title="Xóa" class="pd-setting-ed" onclick="deleteID('+ response.danh_muc_bai_viet.id + ');">'
            	...
            	row.push({
            		'state': '',
            		'id': response.danh_muc_bai_viet.id,
            		'ten': response.danh_muc_bai_viet.ten,
            		'option': strOption,
            	})
...
function editID(id, thisbutton){
    $.ajax({
    	type: 'GET',
    	url: 'quantri/danhmucbaiviet/chinhsua/' + id,
    	success: (response) => {
    		if(response.status){
    			let str = ''
				str += '<div class="modal-dialog">'
					str += '<div class="modal-content">'
						str += '<div class="modal-header header-color-modal bg-color-3" style="background: #2b982b; color: white;">'
							str += '<h4 class="modal-title">Chỉnh sửa danh mục bài viết</h4>'
							str += '<div class="modal-close-area modal-close-df">'
								str += '<a class="close" data-dismiss="modal" href="javascript:void(0)"><i class="fa fa-close"></i></a>'
							str += '</div>'
						str += '</div>'
						str += '<div class="modal-body modal-add">'
							str += '<form id="form-edit" method="POST" enctype="application/x-www-form-urlencoded">'
								str += '<div class="row">'
									str += '<div class="form-group col-md-12">'
										str += '<label>Tên danh mục bài viết</label>'
										str += '<input type="text" class="form-control" placeholder="Nhập tên danh mục bài viết" name="ten" onfocus="focusten(this)" value="' + response.danh_muc_bai_viet.ten + '">'
										str += '<p class="text-danger" id="validateEmpty" style="display:none">Vui lòng điền vào trường này</p>'
										str += '<p class="text-danger" id="validateMaxLength" style="display:none">Tên không được vượt quá 100 ký tự</p>'
									str += '</div>'
								str += '</div>'
								str += '<div class="row">'
									str += '<div class="form-group col-md-12 text-center">'
										str += '<button class="btn btn-success" type="button" onclick="saveEdit(' + response.danh_muc_bai_viet.id + ', ' + $(thisbutton).parent().parent().data('index') + ')">Lưu lại</button>'
									str += '</div>'
								str += '</div>'
							str += '</form>'
						str += '</div>'
...
function deleteID(id) {
    ...
    .then((isConfirm) => {
        if (isConfirm) {
            $.ajax({
                type: 'GET',
                url: 'quantri/danhmucbaiviet/xoa/' + id,
                success: (response) => {
                    if(response.status){
